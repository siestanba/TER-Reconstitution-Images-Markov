...
def charger_image_rgb(path):
    ...
    # Conversion RGB → classe
    h, w, _ = img_np.shape
    champ = np.zeros((h, w), dtype=int)

    for rgb, label in palette.items():
        masque = np.all(img_np == rgb, axis=2)
        champ[masque] = label
...
def gibbs_recuit(champ, nb_iter, modele):
    ...
    for k in tqdm(range(nb_iter), desc="Gibbs recuit simulé (Potts)"):
        T = 1 / np.log(2 + k) # On fait baisser la température lentement
        i = np.random.randint(0, H)
        j = np.random.randint(0, L)
        energies = np.array([
            cdf_locale_8(i, j, H, L, champ, s, modele['poids_aretes'], modele['poids_sommets']) 
...
def afficher_resultats(img_init, img_bruitee, gibbs, gibbs_rec, metro, metro_rec, nb_etats):
    def to_image(img):
        palette = np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255]])  # 3 couleurs RGB
        h, w = img.shape
...
# === Exécution ===
img = charger_image_rgb(chemin_image)
